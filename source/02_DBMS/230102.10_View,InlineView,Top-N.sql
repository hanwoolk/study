...
DROP VIEW EMPv0;
  ...
  SELECT *FROM USER_VIEWS;
  SELECT * FROM EMPv0;
  INSERT INTO EMPv0 VALUES ( 1111,'홍',NULL,NULL,SYSDATE, NULL,NULL,40);
  SELECT *FROM EMP;
...
-- WITH READ ONLY 추가 : 읽기 전용 뷰
  ...
  -- EX. EMP 테이블의 30번 부서의 가상의 테이블 (30번만 DML가능)
CREATE OR REPLACE VIEW EMPv0
    AS SELECT*FROM EMP WHERE DEPTNO=30
    WITH CHECK OPTION;
...
SELECT *FROM EMPv0;
INSERT INTO EMPv0 VALUES (1112, '홍', 'MANAGER', 10, 'SALES');
...
-- EX1. 사번, 이름, 급여, 10의자리에서 반올림한 급여를 뷰로 생성(EMPv0)

CREATE OR REPLACE VIEW EMPv0
    AS SELECT EMPNO, ENAME, SAL, ROUND(SAL,-2) ROUND_SAL FROM EMP;
    SELECT * FROM EMPv0;
    INSERT INTO EMPv0 VALUES ( 1114, 'KIM', 1800, 2000); -- 복합뷰는 INSERT 불가
    
-- EX2. 중복이 제거된 JOB,DEPTNO를 뷰로 생성  DISTINCT
CREATE OR REPLACE VIEW EMPv0
...
SELECT AVG(SAL) FROM (SELECT SAL FROM EMP WHERE SAL>2000) A;
    -- EX. 부서 평균 월급보다 월급이 높은 사원을 사번, 이름, 급여, 부서번호, 해당 부서의 평균(반올림)
