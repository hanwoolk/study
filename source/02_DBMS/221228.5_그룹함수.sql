...
SELECT ROUND(AVG(SAL),1), SUM(SAL), MIN(SAL), MAX(SAL), 
    ...
    FROM EMP;
SELECT AVG(SAL) - 2*STDDEV(SAL), AVG(SAL) + 2*STDDEV(SAL) FROM EMP; -- 95.44%의 SAL 추출

--?	탄탄 다지기
-- 탄탄ex1. 입사한지 가장 오래된 사원의 입사일과 가장 최근에 입사한 사원의 입사일을 출력. 

-- 탄탄ex2. (결과) 80/12/17:14,620일째 83/01/12:13,864일째
SELECT MIN(HIREDATE)||':'||TO_CHAR(TRUNC(SYSDATE-MIN(HIREDATE)),'99,999')||'일째' FIRST,
       MAX(HIREDATE)||':'||TO_CHAR(TRUNC(SYSDATE-MAX(HIREDATE)),'99,999')||'일째' LAST
    FROM EMP;


-- 탄탄ex3.  (결과) 80년12월17일 최초입사:14,620일째 83년01월12일 최근입사 :13,864일째
SELECT TRIM(TO_CHAR(MIN(HIREDATE),'RR"년"MM"월"DD"일 최초입사:"'))||
       TRIM(TO_CHAR(TRUNC(SYSDATE-MIN(HIREDATE)),'99,999'))||'일째' FIRST,
       TRIM(TO_CHAR(MAX(HIREDATE),'RR"년"MM"월"DD"일 최근입사:"'))||
       TRIM(TO_CHAR(TRUNC(SYSDATE-MAX(HIREDATE)),'99,999'))||'일째' LAST
       FROM EMP;           -- TO_CHAR가 연결 연산자를 만나면 ' ' 자동 추가
-- 탄탄ex4. 10번 부서 소속의 사원 중에서 커미션을 받는 사원의 수를 구해 보시오.
SELECT COUNT(NVL(COMM,0)) "COMM EMP" FROM EMP WHERE DEPTNO = 10 AND NVL(COMM,0)>0;
--   3                                   1       2
...
SELECT DEPTNO, MAX(SAL), AVG(SAL) 
    ...
    GROUP BY DEPTNO 
    ORDER BY DEPTNO;
    --GROUP BY 절에서는 컬럼의 별칭을 사용할 수 없다(ORDER BY 절에서만 컬럼별칭 사용가능)
...
SELECT DNAME, MAX(SAL), AVG(SAL) AVG_SAL
    ...
    -- EX. SAL이 5000미만인 사원에대해서만 부서번호별 사원수, 최대급여, 최소급여, 평균급여(부서번호 순 정렬)
SELECT DEPTNO,COUNT(*), MAX(SAL), MIN(SAL), ROUND(AVG(SAL),1) AVG_SAL
    FROM EMP
    WHERE SAL<5000
    GROUP BY DEPTNO
    ORDER BY DEPTNO;
...
SELECT DEPTNO, AVG(SAL)         -- 3
    FROM EMP                    -- 1 
    GROUP BY DEPTNO             -- 2
    HAVING AVG(SAL)>2000        -- 4 , HAVING 절에서 필드의 별칭 사용 불가
    ORDER BY DEPTNO;            -- 5 , 필드의 별칭은 ORDER BY 절에서만 사용 가능
    -- EX. SAL이 5000미만인 사원에 대해 부서명별 최소급여, 최대급여, 평균급여
        -- (단, 평균 급여가 1800이상인 부서명에 대해서 평균급여순으로 정렬 출력)
SELECT DNAME, MIN(SAL), MAX(SAL), AVG(SAL) AVG_SAL
    FROM EMP E, DEPT D
...
SELECT MIN(SAL),MAX(SAL),MAX(SAL)-MIN(SAL)
    FROM EMP;
    
-- 4. 각 부서별로 인원수, 급여 평균, 최저 급여, 최고 급여, 급여의 합을 출력(급여의 합이 많은 순으로)
SELECT DNAME, COUNT(*), ROUND(AVG(SAL),1) "AVG(SAL)", MIN(SAL), MAX(SAL), SUM(SAL)
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
    GROUP BY DNAME
...
SELECT JOB, ROUND(AVG(SAL),1) "AVG(SAL)", SUM(SAL)
    FROM EMP
    GROUP BY JOB
...
SELECT DEPTNO, ROUND(AVG(SAL),1) "AVG(SAL)", SUM(SAL), MIN(SAL)
    FROM EMP
    GROUP BY DEPTNO;
    
--12. 위의 11번을 수정하여, 부서별 급여평균 최대치, 부서별 급여합의 최대치, 부서별 최소급여의 최대치를 출력
SELECT MAX(ROUND(AVG(SAL),1)) "SAL(AVG_MAX)", MAX(SUM(SAL)) "SAL(SUM_MAX)", MAX(MIN(SAL)) "SAL(MIN_MAX)"
    FROM EMP
    GROUP BY DEPTNO;
...
SELECT NVL(TO_CHAR(EXTRACT(YEAR FROM HIREDATE)),'TOTAL') H_YEAR, COUNT(*)
    FROM EMP
    GROUP BY ROLLUP (EXTRACT(YEAR FROM HIREDATE));
--15. 최대급여, 최소급여, 전체급여합, 평균을 출력
SELECT MAX(SAL), MIN(SAL), SUM(SAL), ROUND(AVG(SAL),1) "AVG(SAL)"
    FROM EMP;
