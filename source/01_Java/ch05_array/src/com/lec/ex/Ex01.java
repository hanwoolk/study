...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 1. 배열 변수 선언과 초기화 - 객체(참조)
		int[] iArr = { 10, 20, 30, 40, 50 }; // 0번째,1번째,2번째,...
		iArr[2] = 300; // 배열은 index로 접근(0~4)
		for (int idx = 0; idx < iArr.length; idx++) { // length 배열 방의 갯수
			System.out.println(iArr[idx]);
		}
		// 2. 배열 변수 선언과 배열 메모리 공간 확보
		int[] iArr2 = new int[5]; // new : 메모리 공간을 확보, {0,0,0,0,0} 상태
		iArr2[0] = 999;
		...
		// 확장 for문
		for (int temp : iArr3) {
			System.out.println(temp); // index랑 같이 출력이 필요하면 일반 for문 필요
		}
