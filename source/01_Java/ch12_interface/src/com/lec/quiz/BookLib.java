...
public class BookLib extends BookInfo implements ILendable  {
	...
	public void checkOut(String borrower, String checkOutDate) {
		if(state == STATE_BORROWED) { // 대출중이면 메세지 뿌리고 끝
			System.out.println(getBookTitle() +"는 대출중입니다");
			return;	//호출한 곳으로 돌아감
		}
		...
		System.out.println(getBookTitle() + "도서가 대출처리 되었습니다.");
		System.out.println("대출인 : " + borrower + "\t대출일 : " + checkOutDate);
	}
	@Override
	public void checkIn() {
		if(state == STATE_NORMAL) {
			System.out.println(getBookTitle()+"도서는 반납 완료된 책입니다. 확인 필요.");
			return;
		}
...
//		}
		String msg = getBookNo() + "\t" + getBookTitle() + "(" + getWriter() + " 저)-";
