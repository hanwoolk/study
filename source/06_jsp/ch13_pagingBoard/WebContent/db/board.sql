...
CREATE TABLE BOARD(
    ...
    READCOUNT   NUMBER(5,0) DEFAULT 0,      -- 글 HIT수 (조회수)
    ...
    RE_INDENT NUMBER(2)     NOT NULL,       -- 글 LIST 출력시 글 제목 들여쓰기 정도(원글0)   
...
SELECT * 
    ...
    WHERE RN BETWEEN 11 AND 20; -- 완성(DAO에 들어갈 sql문)
-- 3. 글쓰기(원글쓰기) - 글쓴이 , 글제목, 글본문, EMAIL, PW
  -- 글번호 생성
  SELECT NVL(MAX(NUM),0)+1 FROM BOARD;
INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
    VALUES((SELECT NVL(MAX(NUM),0)+1 FROM BOARD),'홍길동','제목1','본문\n방가',null,'111',(SELECT NVL(MAX(NUM),0)+1 FROM BOARD), 0, 0, '192.169.0.1');
