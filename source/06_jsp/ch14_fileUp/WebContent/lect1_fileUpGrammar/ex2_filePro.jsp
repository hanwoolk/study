	...
	<%
		...
		try {
			...
			while(paramNames.hasMoreElements()){
				...
				filenames[idx] = multiRequest.getFilesystemName(param); // 해당 하라미터 이름으로 저장된 파일
				oriFilenames[idx] = multiRequest.getOriginalFileName(param);
				System.out.println(idx+"번째 파라미터 : " +param + ", 서버에 저장된 파일 이름 : " + filenames[idx] + "오리지널 파일 이름 : " + oriFilenames[idx]);
				idx++;
			}
			name = multiRequest.getParameter("name");
			//Dao 의 함수 호출하여 DB저장(간단한경우 try 안에서, 긴 경우 try 밖에서)
		}catch(IOException e){
			...
			if(file!=null){
				...
				try{
					...
					if(serverFile.exists()){
						...
						os = new FileOutputStream("D:/KHW/source/06_jsp/ch14_fileUp/WebContent/fileUpFolder/"+file);
