...
//	-- 8. 답변글 저장 전 단계(엑셀에서 ⓐ단계)
	private void perReplyStep(int ref, int re_step) {
	Connection conn = null;
	PreparedStatement pstmt = null;
...
//	-- 9. 답변글 쓰기	
	public int reply(BoardDto dto) {
		// num, wrtier, subject, content, email, readcount, pw, ref, re_step, re_ident, ip
		// 원글에 대한 정보 : num, ref, re_step, re_indent
		// 사용자로부터 입력받는 내용 : writer, subject, content, email, pw
		// request.getRemoteAddr() 함수로부터 ip 추출
		perReplyStep(dto.getRef(), dto.getRe_indent()); //답글쓰기 전단계
		int result = FAIL;
		Connection 			conn  = null;
		PreparedStatement 	pstmt = null;
		String sql = "INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP,RE_INDENT,IP)" + 
