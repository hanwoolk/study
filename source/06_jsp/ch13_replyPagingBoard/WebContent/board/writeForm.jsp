...
<body>
	<%
		...
		if(request.getParameter("num")!=null){
			...
			ref = dto.getRef();							//권글의 ref
			re_step = dto.getRe_step();			//원글의 re_step
			re_indent = dto.getRe_indent();		//원글의 re_indent
			System.out.print("writeFrom(답변쓰기)에서 넘어온 원글의 ref : " + ref);
			System.out.print("writeFrom(답변쓰기)에서 넘어온 원글의 re_step : " + re_step);
			System.out.print("writeFrom(답변쓰기)에서 넘어온 원글의 re_indent : " + re_indent);
		}
		/////////////////////////////////
	%>
	<form action="<%=conPath %>/board/writePro.jsp" method="post">
		...
		<table>
			...
			<tr>
				<th>본문</th>
				<td><textarea rows="5" cols="10" name="content" required="required"></textarea></td>
			</tr>
			<tr>
				<th>이메일</th>
