    ...
    _article = 
      <CreateContent
        onCreate={(_title, _body) => {
          // alert(_title + '/' + _body);
          // nextId++ ; {id:4, title:_title, body:_body}객체를 contents에 추가
              ...
              for(var idx=0; idx<_contents.length; idx++){
                if(_contents[idx].id === selectedId){
                  ...
                  if(_contents.length>0){
                    // _id = _contents[0].id; //제일작은 id값을 selectedId로
                    _id = _contents[_contents.length-1].id; //제일 큰 id값을 selecedId로
                  }else{
                    _id =0;
                  }// if
                  setContents(_contents);
