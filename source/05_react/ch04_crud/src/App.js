  ...
  getContent(){
    ...
    if (this.state.mode === "welcome") {
      ...
      _article = <CreateContent onChangePage={function(_title, _desc){
        // alert(_title + '/'+ _desc);
        // this.state.contents.push(id:max_content_id+1, title:_tile, desc:_desc)를 push
        this.max_content_id ++;
        // this.state.contents.push({ //state의 값을 변경하려면 반드시 setState()함수를 이용
      ...
      _article = <UpdateContent data={_content} onChangePage={function(_id, _title, _desc){
        //id가 _id인 contents안의 객체값을 교페 ({id:_id, title:_title, desc: _desc})
            ...
            if(window.confirm('삭제하시면 복구 불가합니다. 삭제 진행할까요?')){
              ...
              for(var idx=0; idx<_contents.length; idx++){
                if(_contents[idx].id === this.state.selected_content_id){
                  ...
                  var first; // id값을 기준으로 오름차순 정렬 후 첫번째 객체의 id
                  if(_contents.length>0){
                    first = Array.from(_contents).sort(function(left,right){
                            return left.id - right.id; // 오른차순 정렬
                            }).slice(0,1)[0].id;
                  }else{
                    first=0;
                  }//if
