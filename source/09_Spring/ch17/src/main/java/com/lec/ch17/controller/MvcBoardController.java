...
import com.lec.ch17.service.*;
import com.lec.ch17.util.Constant;
...
public class MvcBoardController {
	...
	public String reply(int bid, Model model) {
		model.addAttribute("bid",bid);
		bservice = new BmodifyReplyViewService(); //bid로 dto 를 model에 add(board라는 이름으로)
		bservice.execute(model);
		return "mvcBoard/reply";
	}
	@RequestMapping(value="reply", method=RequestMethod.POST)
	public String reply_post(BoardDto boardDto, HttpServletRequest request, Model model) {
		model.addAttribute("request",request);
