...
SELECT PNUM FROM RECTEAM WHERE rId='OP1';
-- (2) 회원가입
...
UPDATE RECTEAM SET rNAME = '0',
                   rPW   = '0'
            WHERE rID='OP1';
SELECT * FROM RECTEAM WHERE rJOB='OPERATOR' ORDER BY rname desc ,rid;
commit;
SELECT rID FROM RECTEAM WHERE rNAME='0' AND rJOB='OPERATOR' ORDER BY rID;
-- (4)프로젝트 등록
...
SELECT *
    FROM (SELECT ROWNUM RN, A.* FROM(SELECT * FROM RECTEAM ORDER BY pNUM DESC) A)
    WHERE RN BETWEEN 1 AND 10 AND PNUM = 2 AND RJOB = 'OPERATOR';
SELECT RID, RNAME, RJOB, P.PNAME
    FROM PROJECT P, RECTEAM R 
    WHERE P.PNUM = R.PNUM AND P.PNUM=1 AND RJOB = 'OPERATOR';
    
SELECT *
    FROM (SELECT ROWNUM RN, A.* FROM(SELECT * FROM MEMBER ORDER BY MID DESC) A)
    WHERE RN BETWEEN 1 AND 10 AND PNUMREG = 2 AND PNUM IS NULL;
...
UPDATE PROJECT SET PSTATUS = '완료' WHERE PNUM=1;
...
SELECT RID, RNAME, RJOB, P.PNAME  
				    FROM PROJECT P, RECTEAM R
				    WHERE P.PNUM = R.PNUM AND P.PNUM=1 AND RJOB = 'OPERATOR';
SELECT * FROM MEMBER WHERE PNUM=1;
ROLLBACK;
-- (4) 신청자 프로젝트 중 퇴출
...
-------------------------------------------------------------------------------
SELECT * FROM MEMBER WHERE PNUM = 1;
SELECT * FROM RECTEAM WHERE PNUM = 1;
SELECT * FROM PROJECT;
-- (1-1) 진행중 프로젝트 목록(최신 등록순)
SELECT *
    FROM (SELECT ROWNUM RN, A.* FROM(SELECT * FROM PROJECT WHERE NOT pSTATUS IN ('완료') ORDER BY PRDATE DESC) A)
    WHERE RN BETWEEN 1 AND 10;
-- (1-2) 완료된 프로젝트 목록(최신 등록순)
SELECT *
    FROM (SELECT ROWNUM RN, A.* FROM(SELECT * FROM PROJECT WHERE pSTATUS = '완료' ORDER BY PRDATE DESC) A)
    WHERE RN BETWEEN 1 AND 10;
-- (2-1) 진행중 프로젝트 갯수
SELECT COUNT(*) FROM PROJECT WHERE NOT PNUM IN (0);
-- (2-2) 완료 프로젝트 갯수
SELECT COUNT(*) FROM PROJECT WHERE pSTATUS = '완료';
...
UPDATE RECTEAM SET PNUM = NULL WHERE PNUM = 1;
UPDATE MEMBER SET PNUM = NULL WHERE PNUM = 1;
DELETE FROM PROJECT WHERE PNUM =1;
ROLLBACK;